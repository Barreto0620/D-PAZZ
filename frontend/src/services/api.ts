...
// Funções de Produtos e Clientes (já conectadas)
export const getProducts = async (): Promise<Product[]> => { const { data, error } = await supabase.from('produtos').select('*, categorias(nome), marcas(nome)'); if (error) { console.error('Erro ao buscar produtos:', error); throw error; } return data.map((p: any) => ({ id: p.id, name: p.nome, description: p.descricao, price: p.preco, oldPrice: p.preco_antigo, images: p.imagens || [], stock: p.estoque, rating: p.avaliacoes, reviewCount: p.numero_avaliacoes, featured: p.em_destaque, onSale: p.em_promocao, bestSeller: p.mais_vendido, category: p.categoria_id, categoryName: p.categorias?.nome || 'Sem Categoria', brand: p.marcas?.nome || 'Sem Marca', color: p.cor, tamanhos: p.tamanhos || [], criado_em: p.criado_em, })); };
export const getProductById = async (id: string): Promise<Product | null> => { const { data, error } = await supabase.from('produtos').select('*, categorias(nome), marcas(nome)').eq('id', id).single(); if (error || !data) { console.error(`Erro ao buscar produto ${id}:`, error); return null; } const p = data; return { id: p.id, name: p.nome, description: p.descricao, price: p.preco, oldPrice: p.preco_antigo, images: p.imagens || [], stock: p.estoque, rating: p.avaliacoes, reviewCount: p.numero_avaliacoes, featured: p.em_destaque, onSale: p.em_promocao, bestSeller: p.mais_vendido, category: p.categoria_id, categoryName: (p.categorias as any)?.nome || 'Sem Categoria', brand: (p.marcas as any)?.nome || 'Sem Marca', color: p.cor, tamanhos: p.tamanhos || [], criado_em: p.criado_em }; };
export const createProduct = async (productData: Omit<Product, 'id' | 'categoryName' | 'brand'>): Promise<Product> => { const { data, error } = await supabase.from('produtos').insert({ nome: productData.name, descricao: productData.description, preco: productData.price, preco_antigo: productData.oldPrice, imagens: productData.images, estoque: productData.stock, avaliacoes: productData.rating, numero_avaliacoes: productData.reviewCount, em_destaque: productData.featured, em_promocao: productData.onSale, mais_vendido: productData.bestSeller, categoria_id: productData.category, cor: productData.color, tamanhos: productData.tamanhos, }).select().single(); if (error) { console.error('Erro ao criar produto:', error); throw error; } const newProduct = await getProductById(data.id); if (!newProduct) throw new Error("Falha ao buscar produto recém-criado."); return newProduct; };
export const updateProduct = async (productId: string, productData: Partial<Product>): Promise<Product | null> => { const { data, error } = await supabase.from('produtos').update({ nome: productData.name, descricao: productData.description, preco: productData.price, preco_antigo: productData.oldPrice, imagens: productData.images, estoque: productData.stock, avaliacoes: productData.rating, numero_avaliacoes: productData.reviewCount, em_destaque: productData.featured, em_promocao: productData.onSale, mais_vendido: productData.bestSeller, categoria_id: productData.category, cor: productData.color, tamanhos: productData.tamanhos, atualizado_em: new Date().toISOString(), }).eq('id', productId).select().single(); if (error) { console.error('Erro ao atualizar produto:', error); throw error; } const updatedProduct = await getProductById(data.id); return updatedProduct; };
...
export const createCategory = async (categoryData: Partial<Omit<Category, 'id' | 'slug'>>): Promise<Category> => {
  const { data, error } = await supabase.rpc('create_new_category', {
    ...
    p_image_url: categoryData.image,
    p_featured: categoryData.featured,
    p_show_in_header: categoryData.showInHeader
  }).single();
...
export const updateCategory = async (categoryId: string, categoryData: Partial<Omit<Category, 'id' | 'slug'>>): Promise<Category> => {
  ...
  if (error) { console.error('Erro ao atualizar categoria (RPC):', error); throw error; }
  return data as Category;
};
export const deleteCategory = async (categoryId: string): Promise<boolean> => {
  const { error } = await supabase.rpc('delete_category_by_id', { p_id: categoryId });
  if (error) { console.error('Erro ao deletar categoria (RPC):', error); return false; }
  return true;
};

// Funções Mock restantes (Intactas)
...
export const getCategoryById = async (id: string): Promise<Category | null> => { await delay(200); const allCategories = await getCategories(); return allCategories.find(c => c.id === id) || null; };
export const getFeaturedCategories = async (): Promise<Category[]> => { await delay(300); const allCategories = await getCategories(); return allCategories.filter(c => c.featured); };
